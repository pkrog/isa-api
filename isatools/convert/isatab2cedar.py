...
CEDAR_SCHEMA_PATH = os.path.join(os.path.dirname(os.path.realpath(__file__)), 
                                 '../resources/schemas/cedar')
...
class ISATab2CEDAR(object):
    ...
    def createCEDARjson_folder(self, work_dir, json_dir, inv_identifier):
        log.info('Convert ISA datasets in folder ', work_dir)
        path = os.path.abspath(work_dir)
        folders = [f for f in os.listdir(path) if
                   os.path.isdir(os.path.join(path, f))]

        for folder in folders:
            self.createCEDARjson(os.path.join(path, folder), json_dir,
                                 inv_identifier)

    def createCEDARjson(self, work_dir, json_dir, inv_identifier):
        log.info('Converting ISA to CEDAR model for ', work_dir)
        schema_file = 'investigation_template.json'
        with open(os.path.join(CEDAR_SCHEMA_PATH, schema_file)) as json_fp:
            schema = json.load(json_fp)
        if schema is None:
            raise IOError('Could not load schema from {}'.format(
                os.path.join(CEDAR_SCHEMA_PATH, schema_file)))
        resolver = RefResolver(str('file://{}'.format(
            os.path.join(CEDAR_SCHEMA_PATH, schema_file))), schema)
        validator = Draft4Validator(schema, resolver=resolver)

        isa_tab = isatab_parser.parse(work_dir)

        if isa_tab is None:
            log.warn('No ISA-Tab dataset found')
        else:
            if isa_tab.metadata != {}:
                investigationObject = dict([
                    ...
                    ('@context', dict([
                        ...
                        ('title',
                         'https://metadatacenter.org/schemas/title'),
                        ('study',
                         'https://metadatacenter.org/schemas/study'),
                        ('submissionDate',
                    ...
                    ('title', dict([('_value', isa_tab.metadata[
                        'Investigation Title'])])),
                    ('description', dict([('_value', isa_tab.metadata[
                        'Investigation Description'])])),
                    ('identifier', dict([('_value', isa_tab.metadata[
                        'Investigation Identifier'])])),
                    ('submissionDate', dict([('_value', isa_tab.metadata[
                        'Investigation Submission Date'])])),
                    ('publicReleaseDate', dict([('_value', isa_tab.metadata[
                        'Investigation Public Release Date'])])),
                    ('study', self.createStudiesList(isa_tab.studies))
                ])
            else:
                investigationObject = dict([
                    ('@id', 'https://repo.metadatacenter.org/UUID'.format(
                        uuid.uuid4())),
                    ('_templateId', 'http://example.org'),
                    ('@type',
                     'https://repo.metadatacenter.org/model/Investigation'),
                    ('@context', dict([
                        ...
                        ('study',
                         'https://metadatacenter.org/schemas/study'),
                        ('submissionDate',
                         'https://metadatacenter.org/schemas/submissionDate'
                         ),
                        ('_value', 'https://schema.org/value'),
                        ('publicReleaseDate',
                         'https://metadatacenter.org/schemas/'
            ...
            try:
                investigation_identifier = isa_tab.metadata[
                    'Investigation Identifier']
            except KeyError:
                investigation_identifier = ''

            try:
                study_identifier = isa_tab.studies[0].metadata[
                    'Study Identifier']
                # study_identifier = study_identifier[study_identifier
                # .find('/')+1:]
            except KeyError:
                study_identifier = ''
            ...
            # save output json
            if inv_identifier:
                file_name = os.path.join(json_dir, '{}.json'.format(
                    investigation_identifier))
            else:
    ...
    def createStudiesList(self, studies):
        ...
        for study in studies:
            ...
            json_item = dict([
                ('@id',
                 'https://repo.metadatacenter.org/UUID{}'.format(uuid.uuid4())),
                ('@type', 'https://repo.metadatacenter.org/model/Study'),
                ('title', dict([('_value', study.metadata['Study Title'])])),
                ('description',
                 dict([('_value', study.metadata['Study Description'])])),
                ('identifier',
                 dict([('_value', study.metadata['Study Identifier'])])),
                ('submissionDate',
                 dict([('_value', study.metadata['Study Submission Date'])])),
                ('publicReleaseDate', dict(
                    [('_value', study.metadata['Study Public Release Date'])])),
                ('studyDesignType', dict([('_value', '')])),
                # dict([('value', study.metadata['Study Public Design Type 
                # Accession Number'])]))
                ('publication',
                 self.createStudyPublicationsList(study.publications)),
                ...
                ('studyGroupPopulation', 
                 self.createStudyGroupList(source_dict)),
                ('studyProtocol', 
                 self.createStudyProtocolList(study.protocols)),
                ('process',
                 self.createProcessList(study.process_nodes, source_dict,
    ...
    def createStudyGroupList(self, source_dict):
        ...
        json_item = dict([
            ...
            ('type', dict([
                ('_value',
                 'http://bioportal.bioontology.org/ontologies/EFO/3232')])),
    ...
    def createProcessList(self, process_nodes, source_dict, sample_dict,
        ...
        for process_node_name in process_nodes:
            try:
                measurement_type = \
                    process_nodes[process_node_name].study_assay.metadata[
            ...
            json_item = dict([
                ...
                ('studyProtocol',
                 self.createExecuteStudyProtocol(process_node_name,
                                                 process_node)),
                ('studyAssay',
                ...
                ('input', self.createInputList(process_node.inputs, source_dict,
                                               sample_dict)),
                ('output',
                 self.createOutputList(process_node.outputs, sample_dict)),
                ('parameterValue',
                 self.createParameterValueList(process_node_name, process_node))
            ])
            json_list.append(json_item)
        return json_list
        ...
        json_item = dict([
            ('@context', dict(
                [
                    ...
                    ('protocolParameter', 
                     'https://metadatacenter.org/schemas/protocolParameter'),
            ...
            ('protocolParameter', 
             self.createProtocolParameterFromNode(process_node))
        ])
        return [json_item]
    ...
    def createProtocolParametersList(self, protocol):
        ...
        parametersURIs = \
            protocol['Study Protocol Parameters Name Term Accession Number']
        index = 0
        if len(parameters) > 0:
            for parameter in parameters.split(';'):
                json_item = dict([
                    ('name', dict([('_value', parameter)])),
                    ('description', (dict([('_value', parametersURIs[index] if 
                    (len(parametersURIs) == len(parameters)) else '')]))),
                ])
                index += 1
                json_list.append(json_item)
        return json_list
            ...
            if header.startswith('Parameter Value'):
                json_item = dict([
                    ('@id', 'https://repo.metadatacenter.org/UUID{}'
                     .format(uuid.uuid4())),
                    ('@type', 
                     'https://repo.metadatacenter.org/model/ProtocolParameter'),
        ...
        for node_index in nodes:
            if nodes[node_index].ntype.endswith('Data File'):
                json_item = dict([
                    ...
                    ('type', dict([
                        ('_value', 'http://purl.obolibrary.org/obo/OBI_0000747')
                    ])),
                    ('description', dict([('_value', '')]))
                ])
        ...
        for node_index in nodes:
            if nodes[node_index].ntype == 'Sample Name':
                json_item = dict([
                    ...
                    ('source', dict([('_value', '')])),
                    ('factorValue', 
                     self.createFactorValueList(nodes[node_index])),
                    ('studyTime', self.createStudyTimeCollection()),
                    ('characteristic',
                     self.createCharacteristicList(node_index,
                                                   nodes[node_index]))])
                json_dict.update({node_index: json_item})
        return json_dict
        ...
        for node_index in nodes:
            if nodes[node_index].ntype == "Source Name":
                json_item = dict([
                    ("name", dict([("_value", node_index)])),
                    ("type", dict([
                        ("_value", "http://purl.obolibrary.org/obo/OBI_0000925")
                    ])),
                    ("characteristic", 
                     self.createCharacteristicList(node_index, 
                                                   nodes[node_index])),
        ...
        for header in node.metadata:
            if header.startswith('Characteristics'):
                ...
                json_item = dict([
                    ...
                    ('description', dict([('_value', '')])),
                    ('characteristicValue', 
                     self.createCharacteristicValueList(
                         node.metadata[header][0]))])
                json_list.append(json_item)
        return json_list
    ...
    def createInvestigationContactsList(self, contacts):
        ...
        for contact in contacts:
            json_item = dict([
                ...
                ('lastName', dict([
                    ('_value', contact['Investigation Person Last Name'])])),
                ('firstName', dict([
                    ('_value', contact['Investigation Person First Name'])])),
                ('middleInitial', dict([
                    ('_value', contact['Investigation Person Mid Initials'])])),
                ('email', dict([
                    ('_value', contact['Investigation Person Email'])])),
                ('phone', dict([
                    ('_value', contact['Investigation Person Phone'])])),
                ('fax', dict([
                    ('_value', contact['Investigation Person Fax'])])),
                ('address', dict([
                    ('_value', contact['Investigation Person Address'])])),
                ('role', 
                 dict([
                     ('_value', contact['Investigation Person Roles Term '
                                        'Accession Number'])])),
                ('organization', 
                 self.createAffiliationsList(contact['Investigation Person '
                                                     'Affiliation']))])
            json_list.append(json_item)
        return json_list
    ...
    def createStudyContactsList(self, contacts):
        ...
        for contact in contacts:
            json_item = dict([
                ...
                ('@type', 'https://repo.metadatacenter.org/model/Contact'),
                ('lastName', dict([
                    ('_value', contact['Study Person Last Name'])])),
                ('firstName', dict([
                    ('_value', contact['Study Person First Name'])])),
                ('middleInitial', dict([
                    ('_value', contact['Study Person Mid Initials'])])),
                ...
                ('address', dict([
                    ('_value', contact['Study Person Address'])])),
                ('role', dict([
                    ('_value', 
                     contact['Study Person Roles Term Accession Number'])])),
                ('organization',
                 self.createAffiliationsList(
                     contact['Study Person Affiliation']))])
            json_list.append(json_item)
        return json_list
    ...
    def createInvestigationPublicationsList(self, publications):
        ...
        for publication in publications:
            json_item = dict([
                ...
                ('pubMedID',
                 dict([('_value', publication['Investigation PubMed ID'])])),
                ('dOI', dict([('_value',
                               publication['Investigation Publication DOI'])])),
                ('authorList', self.createAuthorList(
                    publication['Investigation Publication Author List'])),
                ('status', dict([('_value', publication[
    ...
    def createStudyPublicationsList(self, publications):
        ...
        for publication in publications:
            json_item = dict([
                ...
                ('@type', 'https://repo.metadatacenter.org/model/Publication'),
                ('title',
                 dict([('_value', publication['Study Publication Title'])])),
                ('pubMedID', dict([('_value', 
                                    publication['Study PubMed ID'])])),
                ('dOI',
                 dict([('_value', publication['Study Publication DOI'])])),
                ('authorList', self.createAuthorList(
                    publication['Study Publication Author List'])),
                ('status',
                 dict([('_value', publication['Study Publication Status'])]))])
            json_list.append(json_item)
        return json_list
    ...
    def createStudyAssaysList(self, assays):
        ...
        for assay in assays:
            json_item = dict([
                ...
                ('platform', dict([('_value', assay.metadata[
                    'Study Assay Technology Platform'])])),
                ('technology', dict([('_value', assay.metadata[
                    'Study Assay Technology Type'])]))])
            json_list.append(json_item)
        return json_list
    ...
    def createStudyProtocolList(self, protocols):
        ...
        for protocol in protocols:
            json_item = dict([
                ...
                ('description',
                 dict([('_value', protocol['Study Protocol Description'])])),
                ('type', dict([('_value', protocol['Study Protocol Type'])])),
                ('version',
                 dict([('_value', protocol['Study Protocol Version'])])),
                ('uRI', dict([('_value', protocol['Study Protocol URI'])])),
                ('protocolParameter',
                 self.createProtocolParametersList(protocol))])
            json_list.append(json_item)
        return json_list
    ...
    def createStudyFactorsList(self, factors):
        ...
        for factor in factors:
            json_item = self.createStudyFactor(factor['Study Factor Name'],
