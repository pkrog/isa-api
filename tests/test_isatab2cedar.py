...
def setUpModule():
    if not os.path.exists(utils.DATA_DIR):
        raise FileNotFoundError('Could not fine test data directory in {0}. '
                                ...
                                '--single-branch '
                                'git@github.com:ISA-tools/ISAdatasets {0}'
...
class TestIsaTab2Cedar(unittest.TestCase):
    ...
    def test_isatab2cedar_convert_bii_i_1(self):
        ...
        isa2cedar.createCEDARjson(os.path.join(self._tab_data_dir, test_case),
                                  self._tmp_dir, True)
        with open(os.path.join(self._json_data_dir, 'cedar',
                               test_case + '.json')) as expected_file, \
                open(os.path.join(self._tmp_dir,
                                  test_case + '.json')) as actual_file:
    ...
    def test_isatab2cedar_convert_mtbls1(self):
        ...
        with open(os.path.join(self._json_data_dir, 'cedar',
                               test_case + '.json')) as expected_file, \
                open(os.path.join(self._tmp_dir,
                                  '1425901783014' + '.json')) as actual_file:
            expected_json = json.load(expected_file)
            actual_json = json.load(actual_file)
            utils.strip_ids(expected_json)
            utils.strip_ids(actual_json)
    ...
    #     self.folder = os.path.join('/Users/agbeltran/work-dev/isa-explorer/',
    #     'data')
